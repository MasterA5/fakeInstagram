...
<?php
// Fetch comments for desktop side panel
$stmtC = $conn->prepare("SELECT comments.*, users.username, users.avatar FROM comments JOIN users ON comments.user_id = users.id WHERE comments.post_id = ? ORDER BY comments.created_at ASC LIMIT 50");
$stmtC->bind_param("s", $data['id']);
$stmtC->execute();
$allComments = $stmtC->get_result();
$commentCount = $allComments->num_rows;
?>
<div class="card border rounded-none sm:rounded-xl mb-4 animate-slide-up overflow-hidden" style="border-color: var(--border);">
    <div class="xl:flex">
        <!-- IMAGE / MAIN CONTENT -->
        <div class="xl:flex-1 xl:max-w-[50%] xl:border-r" style="border-right-color: var(--border);">
            <div class="p-3 sm:p-4">
                ...
                <?php if (!empty($data['image'])): ?>
                    <div class="-mx-3 sm:-mx-4 mb-3 xl:mx-0" style="background: #000;">
        ...
        <!-- COMMENTS SIDE PANEL (desktop xl+) -->
        <div class="hidden xl:flex xl:flex-col xl:w-[360px] xl:flex-shrink-0">
            <div class="flex-1 overflow-y-auto px-4 py-3 max-h-[500px] space-y-3 comments-container" data-post-id="<?= $data['id'] ?>">
                <?php if ($commentCount > 0): ?>
                    <?php while ($comment = $allComments->fetch_assoc()): ?>
            ...
            <?php if (isset($_SESSION['user_id'])): ?>
                <div class="comment-form flex items-center gap-2 px-4 py-3" style="border-top: 1px solid var(--border);">
